...
from core.setup import initialize_v15
...
from core.timeframes import CFD_TIMEFRAMES, INVESTMENT_TIMEFRAMES, CONSTANT_LEARNING_INTERVALS
...
def show_settings():
    ...
    # Constant Learning Settings
    _display_constant_learning_settings(config)
...
def _display_constant_learning_settings(config: Dict[str, Any]):
    """Display constant learning/autonomous settings."""
    st.subheader("🧠 Constant Learning Engine")

    learning_config = config.setdefault(
        "constant_learning",
        {
            "enabled": True,
            "intervals": CONSTANT_LEARNING_INTERVALS,
            "evaluation_frequency_minutes": 15,
            "autonomous_mode": "monitor-only",
        },
    )

    enabled = st.checkbox(
        "Enable Constant Learning",
        value=learning_config.get("enabled", True),
        help="Enable or disable the autonomous constant learning loop.",
    )
    learning_config["enabled"] = enabled

    intervals = st.multiselect(
        "Select Constant Learning Intervals",
        options=CONSTANT_LEARNING_INTERVALS,
        default=learning_config.get("intervals", CONSTANT_LEARNING_INTERVALS),
        help="Choose which intervals participate in learning and evaluation.",
    )
    learning_config["intervals"] = intervals

    col1, col2 = st.columns(2)
    with col1:
        eval_minutes = st.number_input(
            "Evaluation Frequency (minutes)",
            min_value=1,
            max_value=1440,
            value=int(learning_config.get("evaluation_frequency_minutes", 15)),
            help="How often the autonomous loop evaluates predictions.",
        )
        learning_config["evaluation_frequency_minutes"] = eval_minutes
        learning_config["evaluation_frequency_seconds"] = int(eval_minutes * 60)

    with col2:
        mode_options = ["disabled", "monitor-only", "full-autonomy"]
        current_mode = learning_config.get("autonomous_mode", "monitor-only")
        mode = st.selectbox(
            "Autonomous Operation Mode",
            options=mode_options,
            index=mode_options.index(current_mode) if current_mode in mode_options else 1,
            help="Control whether the engine should run autonomously or stay in monitor mode.",
        )
        learning_config["autonomous_mode"] = mode

    st.caption(
        "Use this section to explicitly enable/disable constant learning, "
        "select active intervals, and configure autonomous operation parameters."
    )

    try:
        engine = ConstantLearningEngine(enabled=enabled)
        status = engine.get_status()
        st.info(
            f"Autonomous status: {'running' if status['running'] else 'stopped'} | "
            f"Intervals: {', '.join(status['active_intervals'])}"
        )
    except Exception as exc:
        st.warning(f"⚠️ Unable to load constant learning engine status: {exc}")
