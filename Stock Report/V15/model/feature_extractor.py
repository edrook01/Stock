...
class FeatureExtractor:
    ...
    def _extract_technical_features(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            features['rsi_14'] = float(rsi(close, period=14).iloc[-1])
        except Exception:
            features['rsi_14'] = 50.0
        
        try:
            features['sma_20'] = float(sma(close, period=20).iloc[-1])
            features['sma_50'] = float(sma(close, period=50).iloc[-1]) if len(df) >= 50 else features['sma_20']
        except Exception:
            features['sma_20'] = float(close.iloc[-1])
            features['sma_50'] = float(close.iloc[-1])
        
        try:
            features['ema_20'] = float(ema(close, period=20).iloc[-1])
            features['ema_50'] = float(ema(close, period=50).iloc[-1]) if len(df) >= 50 else features['ema_20']
        except Exception:
