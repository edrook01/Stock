...
def check_data_locations() -> Dict[str, bool]:
    """
    Check that all data is stored in V15 folder.
    
    Returns:
        Dictionary mapping data type to whether it's in V15 folder
    """
    root = get_root_path()
    
    checks = {
        "config": (get_path('data') / 'config_v15.json').exists(),
...
def verify_portability() -> Dict:
    ...
    all_data_in_V15 = all(data_locations.values())
    no_absolute_paths = len(absolute_path_issues) == 0
    
    return {
        "portable": all_data_in_V15 and no_absolute_paths,
        "absolute_path_issues": absolute_path_issues,
        "data_locations": data_locations,
        "all_data_in_V15": all_data_in_V15,
