...
class TestFunction1TickerAnalysis:
    ...
    def test_data_fetching_all_intervals(self) -> bool:
        ...
        if not DATA_FETCHER_AVAILABLE or not fetch_prices:
            print("  ⚠ Data fetcher not available (skipping)")
            ...
            for interval in self.all_intervals:
                ...
                try:
                    ...
                    if df is not None and len(df) > 0:
                        ...
                        print(f"  ✗ {ticker} @ {interval}: No data returned")
                        
                except Exception as e:
                    print(f"  ✗ {ticker} @ {interval}: Error - {str(e)[:50]}")
        ...
        for interval in self.all_intervals:
            ...
            try:
                ...
                if prediction and isinstance(prediction, dict):
                    ...
                    if has_all_keys:
                        print(f"  ✓ {ticker} @ {interval}: Prediction generated")
                        ...
                        print(f"  ✗ {ticker} @ {interval}: Missing keys: {missing}")
                else:
                    print(f"  ✗ {ticker} @ {interval}: Invalid prediction format")
    ...
    def test_market_sentiment_overview(self) -> bool:
        ...
        if not SENTIMENT_OVERRIDE_AVAILABLE:
            print("  ⚠ Sentiment override not available (skipping)")
            return True  # Not a failure
            ...
            if not SENTIMENT_ANALYZER_AVAILABLE:
                ...
                try:
                    ...
                    if sentiment_score is not None:
                        ...
                        print(f"  ⚠ Sentiment analyzer returned None")
                except Exception as e:
                    print(f"  ⚠ Sentiment analyzer error (may be OK): {str(e)[:50]}")
            ...
            for text, expected_type in test_cases:
                try:
                    ...
                    if score is not None:
                        ...
                        print(f"  ✓ '{text[:30]}...': {detected_type} (score: {score:.2f})")
                        success_count += 1
                    else:
                        print(f"  ⚠ '{text[:30]}...': No score returned")
                        
                except Exception as e:
                    print(f"  ✗ '{text[:30]}...': Error - {str(e)[:50]}")
    ...
    def test_feature_extraction(self) -> bool:
        ...
        try:
            ...
            if not isinstance(features, dict):
                print(f"  ✗ Features not a dict: {type(features)}")
                return False
    ...
    def test_prediction_consistency(self) -> bool:
        ...
        try:
            ...
            if diff < 0.01:  # Very small difference allowed
                print(f"  ✓ Predictions consistent (diff: {diff:.4f})")
                return True
            else:
                print(f"  ⚠ Predictions differ (diff: {diff:.4f})")
                ...
                if interval == "invalid_interval":
                    ...
                    try:
                        ...
                        if df is None or len(df) == 0:
                            ...
                            print(f"    ⚠ Returned data (may be OK)")
                    finally:
                        loop.close()
