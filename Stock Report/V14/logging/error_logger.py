...
def _get_logs_path() -> Path:
    ...
    Resolves to Stock Report/logs/ (parent of V14).
    ...
    if _USE_PORTABLE_PATHS:
        try:
            # portable_paths.get_path('logs') returns V14/logs/, but we need Stock Report/logs/
            # So we'll calculate it manually but use portable_paths for consistency
            v14_root = Path(__file__).parent.parent
            stock_report_root = v14_root.parent
    ...
    # V14 is in Stock Report/V14/, so logs/ is in Stock Report/logs/
    stock_report_root = v14_root.parent
    logs_dir = stock_report_root / 'logs'
